                            ...
                            <table class="table table-condensed">
                                ...
                                <tbody>
                                    ...
                                    foreach ($rows as $row) {
                                        ...
                                        echo '<tr><td colspan="2" class="no-border">#' . $r . ': &nbsp;&nbsp;' . $row->product_code . '</td><td class="no-border text-center" style="width:120px;">' . $this->sma->formatQuantity($row->quantity) . $row->base_unit_code . ' x ' . $this->sma->formatMoney($row->unit_price) . '</td><td class="no-border" style="width:80px;"><span class="pull-right">' . $this->sma->formatMoney($row->subtotal) . '</span></td></tr>';
                                        echo '<tr><td colspan="4" class="no-border" style="padding-top:0;padding-bottom:0;">' . product_name($row->product_name, ($printer ? $printer->char_per_line : null)) . ($row->variant ? ' (' . $row->variant . ')' : '') . '</td></tr>';
                                        if (!empty($row->second_name)) {
                                            echo '<tr><td colspan="4" class="no-border" style="padding-top:0;padding-bottom:0;">' . $row->second_name . '</td></tr>';
                                        }
                                        echo '<tr><td colspan="4" class="no-border border-bottom">' . ($row->item_tax != 0 ? lang('tax') . ' <small>(' . ($Settings->indian_gst ? $row->tax : $row->tax_code) . ')</small> ' . $this->sma->formatMoney($row->item_tax) . ($row->hsn_code ? ' (' . lang('hsn_code') . ': ' . $row->hsn_code . ')' : '') : '') . '</td></tr>';
                                        $r++;
                                    }
                                    if ($return_rows) {
                                        ...
                                        foreach ($return_rows as $row) {
                                            ...
                                            echo '<tr><td colspan="2" class="no-border">#' . $r . ': &nbsp;&nbsp;' . $row->product_code . '</td><td class="no-border text-center">' . $this->sma->formatQuantity($row->quantity) . $row->base_unit_code . ' x ' . $this->sma->formatMoney($row->unit_price) . '</td><td class="no-border"><span class="pull-right">' . $this->sma->formatMoney($row->subtotal) . '</span></td></tr>';
                                            echo '<tr><td colspan="4" class="no-border" style="padding-top:0;padding-bottom:0;">' . product_name($row->product_name, ($printer ? $printer->char_per_line : null)) . ($row->variant ? ' (' . $row->variant . ')' : '') . '</td></tr>';
                                            if (!empty($row->second_name)) {
                                                echo '<tr><td colspan="4" class="no-border" style="padding-top:0;padding-bottom:0;">' . $row->second_name . '</td></tr>';
                                            }
                                ...
                                <tfoot>
                                    ...
                                    if ($inv->order_discount != 0) {
                                        echo '<tr><th colspan="3">' . lang('order_discount') . '</th><th class="text-right">' . $this->sma->formatMoney($inv->order_discount) . '</th></tr>';
                                    }
                                    ...
                                    if ($Settings->indian_gst) {
                                        if ($inv->cgst > 0) {
                                            ...
                                            echo '<tr><td colspan="3">' . lang('cgst') . '</td><td class="text-right">' . ($Settings->format_gst ? $this->sma->formatMoney($cgst) : $cgst) . '</td></tr>';
                                        }
                                        if ($inv->sgst > 0) {
                                            $sgst = $return_sale ? $inv->sgst + $return_sale->sgst : $inv->sgst;
                                            echo '<tr><td colspan="3">' . lang('sgst') . '</td><td class="text-right">' . ($Settings->format_gst ? $this->sma->formatMoney($sgst) : $sgst) . '</td></tr>';
                                        }
                                        if ($inv->igst > 0) {
                                            $igst = $return_sale ? $inv->igst + $return_sale->igst : $inv->igst;
                                            echo '<tr><td colspan="3">' . lang('igst') . '</td><td class="text-right">' . ($Settings->format_gst ? $this->sma->formatMoney($igst) : $igst) . '</td></tr>';
                                        }
                                    }
