...
class Products extends MY_Controller
{
    ...
    public function add_adjustment($count_id = null)
    {
        ...
        if (true == $this->form_validation->run() && $this->products_model->addAdjustment($data, $products)) {
            ...
            $this->data['kategori'] = $this->site->getAllkategoriAdjustment();
    ...
    public function getProducts($warehouse_id = null)
    {
        ...
        if ($warehouse_id) {
            $this->datatables
            ->select($this->db->dbprefix('products') . ".id as productid, {$this->db->dbprefix('products')}.image as image, {$this->db->dbprefix('products')}.code as code, {$this->db->dbprefix('products')}.name as name, {$this->db->dbprefix('brands')}.name as brand, {$this->db->dbprefix('categories')}.name as cname, cost as cost, price as price, COALESCE(wp.quantity, 0) as quantity, {$this->db->dbprefix('units')}.name as unit, wp.rack as rack, alert_quantity", false)
            ->from('products');
        ...
        // ->group_by("products.id");
            ...
            $this->datatables
                ->select($this->db->dbprefix('products') . ".id as productid, {$this->db->dbprefix('products')}.image as image, {$this->db->dbprefix('products')}.code as code, {$this->db->dbprefix('products')}.name as name, {$this->db->dbprefix('brands')}.name as brand, {$this->db->dbprefix('categories')}.name as cname, cost as cost, price as price, COALESCE(quantity, 0) as quantity, {$this->db->dbprefix('units')}.name as unit, '' as rack, alert_quantity", false)
                ->from('products')
