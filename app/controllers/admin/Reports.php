...
class Reports extends MY_Controller
{
    ...
    public function getPurchasesReport($pdf = null, $xls = null)
    {
        ...
        if ($pdf || $xls) {
            ...
            $pi = "( SELECT purchase_id, {$this->db->dbprefix('purchase_items')}.product_id, GROUP_CONCAT(CONCAT({$this->db->dbprefix('purchase_items')}.product_name, '__', {$this->db->dbprefix('purchase_items')}.quantity, (CASE WHEN {$this->db->dbprefix('purchase_items')}.option_id > 0 THEN CONCAT('__',{$this->db->dbprefix('product_variants')}.name) ELSE '' END)) SEPARATOR '___') AS item_nane from {$this->db->dbprefix('purchase_items')} LEFT JOIN {$this->db->dbprefix('product_variants')} ON {$this->db->dbprefix('product_variants')}.id = {$this->db->dbprefix('purchase_items')}.option_id ";
            if ($product) {
    ...
    public function getQuotesReport($pdf = null, $xls = null)
    {
        ...
        if ($pdf || $xls) {
            ...
            $qi = "( SELECT quote_id, {$this->db->dbprefix('quote_items')}.product_id, GROUP_CONCAT(CONCAT({$this->db->dbprefix('quote_items')}.product_name, '__', {$this->db->dbprefix('quote_items')}.quantity, (CASE WHEN {$this->db->dbprefix('quote_items')}.option_id > 0 THEN CONCAT('__',{$this->db->dbprefix('product_variants')}.name) ELSE '' END)) SEPARATOR '___') AS item_nane from {$this->db->dbprefix('quote_items')} LEFT JOIN {$this->db->dbprefix('product_variants')} ON {$this->db->dbprefix('product_variants')}.id = {$this->db->dbprefix('quote_items')}.option_id ";
            if ($product) {
    ...
    public function getSalesTokoReport($pdf = null, $xls = null)
    {
        $this->sma->checkPermissions('sales', true);
        $product = $this->input->get('product') ? $this->input->get('product') : null;
        $user = $this->input->get('user') ? $this->input->get('user') : null;
    ...
    public function toko_sales()
    {
        ...
        $this->data['warehouses'] = $this->site->getAllWarehouses();
        $this->data['billers'] = $this->site->getAllCompanies('biller');
        $bc = [['link' => base_url(), 'page' => lang('home')], ['link' => admin_url('reports'), 'page' => lang('reports')], ['link' => '#', 'page' => 'Laporan Penjualan Pertoko']];
        $meta = ['page_title' => 'Laporan Penjualan Pertoko', 'bc' => $bc];
        $this->page_construct('reports/toko_sales', $meta, $this->data);
    }
